(function(){ try {var elementStyle = document.createElement('style'); elementStyle.appendChild(document.createTextNode("")); document.head.appendChild(elementStyle);} catch(e) {console.error('vite-plugin-css-injected-by-js', e);} })();(function(){"use strict";const k=async(n,a,o)=>(await(await fetch("https://cichautomation.duckdns.org/webhook/9a97cd47-cd26-47ae-91f8-b92d0a62ee5a",{method:"POST",body:JSON.stringify({text:n,sender:a,id:o,timestamp:new Date})})).json()).content,E=async n=>(await(await fetch(`https://cichautomation.duckdns.org/webhook/9a97cd47-cd26-47ae-91f8-b92d0a62ee5a?id=${n}`)).json()).content,u=(n,a)=>`
				...
				</div>`;(async()=>{const n=document.getElementById("AlcLabsScript").getAttribute("data-theme")||"sky",a=document.getElementById("AlcLabsScript").getAttribute("data-apikey"),o=document.createElement("div");document.body.appendChild(o);const l=o.attachShadow({mode:"open"}),r="fixed top-0 pt-20 w-full px-4 z-50 transition-all duration-300 flex justify-center",M="fixed bottom-4 right-4 z-50 transition-all duration-300",g=document.createElement("style"),C="https://cdn.jsdelivr.net/gh/ahroncich7/augmentumResources/styles.css";g.textContent=await fetch(C).then(t=>t.text());const i=document.createElement("div");i.className=r;let d=!0;const T=(t,s)=>`
...
`,h=()=>{i.className=r,i.innerHTML=T(f,n),requestAnimationFrame(()=>{S(),v(),y()})},j=()=>{i.className=M,i.innerHTML=B;const t=i.querySelector("#chat-bubble");t==null||t.addEventListener("click",x)},x=()=>{d?j():h(),d=!d},S=()=>{var s,e;const t=l;(s=t.getElementById("sendButton"))==null||s.addEventListener("click",w),(e=t.getElementById("close-widget"))==null||e.addEventListener("click",x)},w=async()=>{const t=document.getElementById("chat-input"),s=t.value;if(!s)return;b({text:s,sender:"user",timestamp:new Date().toString()}),t.value="";const e=await k(s,"user",a);b(e)},v=()=>{const t=new Date;document.querySelectorAll(".messageTimeStamp").forEach(e=>{const c=new Date(e.dataset.timestamp);if(!c)return;const m=Math.ceil((t-c)/(1e3*60));e.innerHTML=`Hace ${m} min.`})},f=await E(a);l.appendChild(g),l.appendChild(i),h(),document.addEventListener("keydown",t=>{d&&t.key==="Enter"&&!t.shiftKey&&(t.preventDefault(),w())});const b=t=>{const s=document.getElementById("chat-container"),e=t.sender=="user"?p(t.text,t.timestamp,n):u(t.text,t.timestamp),c=document.createElement("div");c.innerHTML=e;const m=c.firstElementChild;s.appendChild(m),f.push(t),requestAnimationFrame(()=>{y(),v()})},y=()=>{const t=document.getElementById("chat-container");t.scrollTo({top:t.scrollHeight,behavior:"smooth"})}})()})();
